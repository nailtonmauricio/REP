    ...
    if($resEmployee->rowCount()>0 && $resRemoteAddress->rowCount()>0)
    {
        ...
        if($resPermission->rowCount()>0)
        {
            ...
            $_SESSION ['msg'] = "<div class='alert alert-danger alert-dismissible text-center'> "
                . "<button type='button' class='close' data-dismiss='alert'>"
                . "<span aria-hidden='true'>&times;</span>"
                . "</button>Permissão para marcação não encontrada</div>";
        }
    }
    else{
        $error = true;
        ...
        $_SESSION ['msg'] = "<div class='alert alert-danger alert-dismissible text-center'> "
            . "<button type='button' class='close' data-dismiss='alert'>"
            . "<span aria-hidden='true'>&times;</span>"
            . "</button>Marcação não permitida a partir deste endereço IP ou senha incorreta</div>";
    }
} else {
    $error = true;
    $_SESSION ['msg'] = "<div class='alert alert-danger alert-dismissible text-center'> "
        . "<button type='button' class='close' data-dismiss='alert'>"
        . "<span aria-hidden='true'>&times;</span>"
        . "</button>Erro ao realizar marcação</div>";
}
if($error){
    //Shipping type error
    header("Location: index.php");
